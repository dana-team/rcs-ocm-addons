...
build-installer: kustomize ## Generate a consolidated YAML with CRDs and deployment.
	mkdir -p dist
	cd score/deploy/resources/default && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build score/deploy/resources/default > dist/score-install.yaml
	cd status/deploy/resources/default && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build status/deploy/resources/default > dist/status-install.yaml
...
deploy-score-addon: kustomize ## Deploy addon to the k8s cluster specified in ~/.kube/config.
	cd score/deploy/resources/default && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build score/deploy/resources/default | kubectl apply -f -
...
undeploy-status-addon: kustomize ## Undeploy addon from the K8s cluster specified in ~/.kube/config. Call with ignore-not-found=true to ignore resource not found errors during deletion.
	$(KUSTOMIZE) build status/deploy/resources/default | kubectl delete --ignore-not-found=$(ignore-not-found) -f -
...
undeploy-score-addon: kustomize ## Undeploy addon from the K8s cluster specified in ~/.kube/config. Call with ignore-not-found=true to ignore resource not found errors during deletion.
	$(KUSTOMIZE) build score/deploy/resources/default | kubectl delete --ignore-not-found=$(ignore-not-found) -f -
